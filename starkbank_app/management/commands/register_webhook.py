...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for webhook in existing_ngrok_webhooks:
            ...
            logger.info(
                f"register_webhook: removed stale webhook {webhook.id} -> {webhook.url}"
            )
        ...
        try:
            ...
            logger.info(
                f"register_webhook: registered webhook {webhook.id} -> {webhook.url}"
            )
            ...
            if any(err.code == "invalidUrl" for err in error.errors):
                ...
                logger.info(
                    f"register_webhook: {target_url} already registered, skipping"
                )
            else:
                raise
    ...
    def _wait_for_ngrok_url(self, attempts=15, delay=2):
        for _ in range(attempts):
            ...
            time.sleep(delay)
        raise RuntimeError(
            "register_webhook: ngrok tunnel did not become available in time"
        )

    def _find_ngrok_webhooks(self, user):
        return [
            webhook
            for webhook in starkbank.webhook.query(user=user)
